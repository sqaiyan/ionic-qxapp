	...
	artDialog.fn = artDialog.prototype = {
		...
		size: function(width, height) {
			...
			if (width) {
				...
				if (typeof width === 'number') {
					...
					style.width = Math.max(that.config.minWidth, Math.min(width, that.config.max_Width)) + 'px';
			...
			if (height) {
				...
				if (typeof height === 'number') {
					height = Math.max(that.config.minHeight, height);
					style.height = height + 'px';
					cntstyle.height = height - btnwrap[0].offsetHeight + 'px';
				} else if (typeof height === 'string') {
					style.height = height;
					cntstyle.height = 'auto';
					if (height != 'auto') {
						cntstyle.height = parseInt(height) - btnwrap[0].offsetHeight + 'px';
					}
				};
		...
		button: function() {
			...
			$.each(list, function(i, val) {
				...
				button.className = 'btn btn_bd btn_bd_primary';
				if (val.className) button.className = 'btn btn_bd btn_bd_primary '+val.className;
...
//				button.disabled = !!val.disabled;
		...
		lock: function() {
			...
			var that = this,
				...
				ie6Css = _isIE6 ?
				'position:absolute;left:expression(' + domTxt + '.scrollLeft);top:expression(' + domTxt + '.scrollTop);width:expression(' + domTxt + '.clientWidth);height:expression(' + domTxt + '.clientHeight)' : '';
	...
	artDialog.templates =
		...
		'<div colspan="2" class="aui_header">' +
		...
		'</v>' +
		'<div colspan="2" class="aui_footer">' +
