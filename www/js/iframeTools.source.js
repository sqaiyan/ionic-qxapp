...
(function($, window, artDialog, undefined) {
			...
			init: function(here) {
				...
				$.ajax({
					...
					error: function(a, b, c, d) {
						api.content('<div class="fc_r txt-c fs_3"><i class="qx qx-warn fs_4 ph"></i>获取内容失败</div>');
						opt.init && opt.init.call(api, here);
					},
					cache: cache
				});
	...
	artDialog.alert = function(content) {
		return _proxyDialog({
			...
			lock: true,
			title: false,
			content:"<div class='txt-c pd fs_2'>"+content+'</div>',
			ok: true,
			width:'90%'
		});
	};
	...
	artDialog.tips = function(content, time) {
		return _proxyDialog({
			...
			.content('<div class="dialog_tips">' + content + '</div>')
			.time(time || 2);
	};
