...
- (void)sendPaymentRequest:(CDVInvokedUrlCommand *)command
{
    ...
    NSString * myDataTime = [self getMytimestamp];
    req.timeStamp = [myDataTime intValue];
    req.openID=params[@"appid"];
   ...
   // req.timeStamp = [params[@"timestamp"] intValue];
    ...
    NSString * myStr = [NSString stringWithFormat:@"appid=%@&noncestr=%@&package=%@&partnerid=%@&prepayid=%@&timestamp=%@&key=8c97e4fa0618742bf9fabfb520ce4ac4",req.openID,req.nonceStr,req.package,req.partnerId,req.prepayId,myDataTime];
    req.sign = [[self md5:myStr] uppercaseString];
